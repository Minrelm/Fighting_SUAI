...
class Fighter():
    def __init__(self, player, x, y, flip, data, sprite_sheet, animation_steps, sound):
        ...
        self.action = 0 #0:бездействие #1:бег #2:прыжок #3:атака1 #4: атака2 #5:урон #6:смерть
        self.frame_index = 0
    ...
    def load_images(self, sprite_sheet, animation_steps):
        #извлечение изображений из подготовленной картинки
        animation_list = []
            ...
            if self.player == 1:
              #движение
                ...
                if key[pygame.K_r] or key[pygame.K_t]:
                    self.attack(target)
                    #определить, какая атака была использована
                    if key[pygame.K_r]:
                        self.attack_type = 1
                    if key[pygame.K_t]:
                        self.attack_type = 2
    ...
    def attack(self, target):
        if self.attack_cooldown == 0:
            ...
            attacking_rect = pygame.Rect(self.rect.centerx - (2 * self.rect.width * self.flip), self.rect.y, 2 *
